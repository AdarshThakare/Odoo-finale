...
export default async function AttendancePage({
	...
	searchParams?: { month?: string; date?: string; search?: string };
}) {
	...
	if (session.user.role === "EMPLOYEE") {
		const monthKey = getMonthKey(searchParams?.month);
		const data = await api.attendance.getMyAttendance({ month: monthKey });
		const prevHref = buildHref("/dashboard/attendance", { month: addMonths(monthKey, -1) });
		const nextHref = buildHref("/dashboard/attendance", { month: addMonths(monthKey, 1) });
	...
	const dateKey = searchParams?.date?.trim() || new Date().toISOString().slice(0, 10);
