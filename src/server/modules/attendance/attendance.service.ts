...
type AttendanceStatusView = "ABSENT" | "PRESENT" | "HALF_DAY" | "ON_LEAVE" | "IN_PROGRESS";
...
export async function getMyAttendanceForUser(
	...
	const [attendanceRecords, employeeProfile] = await Promise.all([
		getAttendanceForEmployeeMonth(db, employee.id, month.startDate, month.endDate),
	...
	const days = enumerateDates(month.startDate, month.endDate).map((date) => {
		...
		const workingHours = record?.workingHours ? Number(record.workingHours) : null;
		const status = record
			? deriveStatus(record)
			: "ABSENT";
	...
	const currentlyCheckedIn = attendanceRecords.some((record) => !record.checkOut && !!record.checkIn);
	...
	return {
		...
		month: {
			...
			endDate: formatDateKey(new Date(month.endDate.getTime() - 24 * 60 * 60 * 1000)),
			totalWorkingDays,
		},
		stats: {
			daysPresent,
...
export async function getAllAttendanceForUser(
	...
	const employees = await getAttendanceForCompanyDate(db, scope.companyId, date, search);
	...
	const presentCount = rows.filter((row) => row.status === "PRESENT" || row.status === "HALF_DAY").length;
...
export async function checkInForUser(db: PrismaClient, userId: string) {
	...
	const existing = await getAttendanceRecordForEmployeeDate(db, employee.id, today);
...
export async function checkOutForUser(db: PrismaClient, userId: string) {
	...
	const today = startOfUtcDay(new Date());
	const record = await getAttendanceRecordForEmployeeDate(db, employee.id, today);
	...
	const now = new Date();
	const hours = Math.max((now.getTime() - record.checkIn.getTime()) / (1000 * 60 * 60), 0);
	const workingHours = new Prisma.Decimal(hours.toFixed(2));
	const status = hours >= WORK_DAY_HOURS ? "PRESENT" : hours >= 4 ? "HALF_DAY" : "ABSENT";
