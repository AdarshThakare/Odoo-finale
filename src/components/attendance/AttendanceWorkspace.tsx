		...
		<div className="min-h-[calc(100vh-4rem)] bg-[#090909] px-4 py-6 text-[#f5f1e8] sm:px-6 lg:px-8">
			<div className="mx-auto flex max-w-[1600px] flex-col gap-6">
				<header className="grid gap-4 rounded-[28px] border border-white/20 bg-white/5 p-4 shadow-[0_24px_80px_rgba(0,0,0,0.35)] backdrop-blur md:grid-cols-[minmax(0,1.3fr)_minmax(0,0.7fr)] md:p-6">
					<div>
						<p className="text-xs uppercase tracking-[0.35em] text-[#9cb3b0]">
							Attendance / My view
						</p>
						...
						<p className="mt-2 max-w-2xl text-sm leading-6 text-[#c9c2b4]">
							{data.company.name} • {data.employee.department} • {data.employee.designation}
						</p>
					</div>
					...
					<div className="grid gap-3 sm:grid-cols-3">
						...
						<StatCard label="Working days" value={String(data.stats.totalWorkingDays)} />
					</div>
				</header>
					...
					<aside className="space-y-6">
						<section className="rounded-[28px] border border-white/20 bg-[#0f0f0f] p-5 shadow-[0_24px_80px_rgba(0,0,0,0.3)]">
							<div className="flex items-center justify-between gap-4 border-b border-white/15 pb-4">
								<div>
									<p className="text-xs uppercase tracking-[0.3em] text-[#8ca3a0]">Note</p>
									<h2 className="mt-1 text-2xl font-semibold text-[#fbf7ee]">Attendance rules</h2>
								</div>
						...
						<section className="rounded-[28px] border border-white/20 bg-[#111111] p-5">
							<div className="flex flex-wrap items-end justify-between gap-4">
								<div>
									<p className="text-xs uppercase tracking-[0.3em] text-[#8ca3a0]">Actions</p>
									<h2 className="mt-1 text-xl font-semibold text-[#faf7ef]">Punch clock</h2>
								</div>
						...
						<div className="flex flex-col gap-4 border-b border-white/15 p-4 sm:p-5">
							...
							<div className="overflow-x-auto">
								<table className="min-w-full border-separate border-spacing-0 text-left text-sm">
									<thead>
										<tr className="text-xs uppercase tracking-[0.24em] text-[#a9a49a]">
											<th className="border-b border-r border-white/10 px-4 py-3">Date</th>
											<th className="border-b border-r border-white/10 px-4 py-3">Check in</th>
											<th className="border-b border-r border-white/10 px-4 py-3">Check out</th>
											<th className="border-b border-r border-white/10 px-4 py-3">Work hours</th>
											<th className="border-b border-r border-white/10 px-4 py-3">Extra hours</th>
									...
									<tbody>
										{data.days.map((day) => (
											<tr key={day.date} className="text-[#efe8db]">
												<td className="border-b border-r border-white/10 px-4 py-3 align-top">
													<div className="font-medium text-[#fbf7ee]">{day.date}</div>
													<div className="text-xs text-[#a9a49a]">{day.weekday}</div>
												</td>
												<td className="border-b border-r border-white/10 px-4 py-3 font-mono">{day.checkIn ?? "—"}</td>
												<td className="border-b border-r border-white/10 px-4 py-3 font-mono">{day.checkOut ?? "—"}</td>
												<td className="border-b border-r border-white/10 px-4 py-3 font-mono">{day.workingHours ?? "—"}</td>
												<td className="border-b border-r border-white/10 px-4 py-3 font-mono">{day.extraHours ?? "—"}</td>
			...
			<div className="mx-auto flex max-w-[1600px] flex-col gap-6">
				<header className="grid gap-4 rounded-[28px] border border-white/20 bg-white/5 p-4 shadow-[0_24px_80px_rgba(0,0,0,0.35)] backdrop-blur md:grid-cols-[minmax(0,1.25fr)_minmax(0,0.75fr)] md:p-6">
					...
					<div className="grid gap-3 sm:grid-cols-4">
						...
						<StatCard label="In progress" value={String(data.stats.inProgressCount)} />
						<StatCard label="Absent" value={String(data.stats.absentCount)} />
						<StatCard label="Employees" value={String(data.stats.totalEmployees)} />
					</div>
				</header>
					...
					<aside className="space-y-6">
						...
						<section className="rounded-[28px] border border-white/20 bg-[#111111] p-5">
							<div className="grid gap-3 sm:grid-cols-2">
								<MiniStat label="Present today" value={String(data.stats.presentCount)} />
								<MiniStat label="Leaves" value={String(data.stats.leaveCount)} />
								<MiniStat label="Selected date" value={data.date.label} />
								<MiniStat label="Employees scanned" value={String(data.stats.totalEmployees)} />
							</div>
						</section>
					</aside>
						...
						<div className="flex flex-col gap-4 border-b border-white/15 p-4 sm:p-5">
							...
							<div className="overflow-x-auto">
								<table className="min-w-full border-separate border-spacing-0 text-left text-sm">
									<thead>
										<tr className="text-xs uppercase tracking-[0.24em] text-[#a9a49a]">
											<th className="border-b border-r border-white/10 px-4 py-3">Emp</th>
											<th className="border-b border-r border-white/10 px-4 py-3">Check in</th>
											<th className="border-b border-r border-white/10 px-4 py-3">Check out</th>
											<th className="border-b border-r border-white/10 px-4 py-3">Work hours</th>
											<th className="border-b border-r border-white/10 px-4 py-3">Extra hours</th>
									...
									<tbody>
										{data.rows.length === 0 ? (
											<tr>
												<td colSpan={6} className="px-4 py-10 text-center text-[#b7b0a3]">
											...
											data.rows.map((row) => (
												<tr key={row.employeeId} className="text-[#efe8db]">
													<td className="border-b border-r border-white/10 px-4 py-3 align-top">
														<div className="font-medium text-[#fbf7ee]">{row.name}</div>
														<div className="text-xs text-[#a9a49a]">{row.employeeCode} · {row.department}</div>
													</td>
													<td className="border-b border-r border-white/10 px-4 py-3 font-mono">{row.checkIn ?? "—"}</td>
													<td className="border-b border-r border-white/10 px-4 py-3 font-mono">{row.checkOut ?? "—"}</td>
													<td className="border-b border-r border-white/10 px-4 py-3 font-mono">{row.workingHours ?? "—"}</td>
													<td className="border-b border-r border-white/10 px-4 py-3 font-mono">{row.extraHours ?? "—"}</td>
													<td className="border-b border-white/10 px-4 py-3">
...
function StatCard({ label, value }: { label: string; value: string }) {
	return (
		<div className="rounded-2xl border border-white/15 bg-black/20 px-4 py-3">
			<p className="text-[11px] uppercase tracking-[0.32em] text-[#8ca3a0]">{label}</p>
...
function MiniStat({ label, value }: { label: string; value: string }) {
	return (
		<div className="rounded-2xl border border-white/15 bg-black/20 px-4 py-3">
			<p className="text-[11px] uppercase tracking-[0.28em] text-[#8ca3a0]">{label}</p>
			<p className="mt-2 text-base font-semibold text-[#faf7ef]">{value}</p>
		</div>
	);
}

function StatusBadge({ status }: { status: EmployeeAttendanceDay["status"] | TeamAttendanceRow["status"] }) {
