...
- [x] **Step 1: Create `src/server/repositories/leave.repo.ts`**
...
- [x] **Step 1: Create `src/server/modules/leave/leave.utils.ts`**
...
- [x] **Step 2: Create `src/server/modules/leave/leave.service.ts`**
...
- [x] **Step 1: Create `src/server/api/routers/leave.ts`**
...
- [x] **Step 2: Wire `leaveRouter` into `src/server/api/root.ts`**
...
- [x] **Step 1: Create `src/app/(dashboard)/dashboard/leave/manage/page.tsx`**
...
- [x] **Step 2: Create `src/components/leave/LeaveManageWorkspace.tsx`**
...
export function LeaveManageWorkspace({
  ...
  const { data: leaveTypes = initial } = api.leave.listTypes.useQuery(
    undefined,
    {
      initialData: initial,
    },
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Configure leave types and allocate quotas to employees
          </p>
        </div>
      </div>
              ...
              <div className="flex gap-4">
                <label className="flex items-center gap-2 text-sm text-gray-700">
                  <input
                    ...
                    onChange={(e) =>
                      setTypeForm((f) => ({ ...f, isPaid: e.target.checked }))
                    }
                    className="rounded"
                  />
                  Paid leave
          ...
          <div className="space-y-3">
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setAllocForm((f) => ({ ...f, employeeId: e.target.value }))
                }
                className="block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-purple-500"
              >
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setAllocForm((f) => ({ ...f, leaveTypeId: e.target.value }))
                }
                className="block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-purple-500"
              >
...
- [x] **Step 1: Create `src/app/(dashboard)/dashboard/leave/page.tsx`**
...
- [x] **Step 2: Create `src/components/leave/MyLeaveWorkspace.tsx`**
...
export function MyLeaveWorkspace({
  ...
  const { data: applications = initial } = api.leave.getMyApplications.useQuery(
    undefined,
    {
      initialData: initial,
    },
    ...
    <div>
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            View your leave balance and applications
          </p>
        </div>
        <div className="flex gap-2">
          {canManage && (
            <Link
              ...
              className="rounded-lg border border-purple-200 px-4 py-2 text-sm font-medium text-purple-700 transition hover:bg-purple-50"
            >
              Manage
            </Link>
          )}
            ...
            >
              ...
              <div className="mt-2 h-1.5 w-full rounded-full bg-gray-100">
                <div
                  ...
                  style={{
                    width: `${b.totalDays > 0 ? (b.remainingDays / b.totalDays) * 100 : 0}%`,
                  }}
                />
              </div>
              <p className="mt-1 text-xs text-gray-400">
                {b.usedDays} used {b.isPaid ? "· Paid" : "· Unpaid"}
              </p>
            </div>
          ))}
        </div>
      ...
      {balances.length === 0 && (
        <div className="mb-6 rounded-xl bg-amber-50 p-4 text-sm text-amber-700 ring-1 ring-amber-200">
          No leave has been allocated to you yet. Contact HR to set up your
          leave balance.
        </div>
      )}
              ...
              {applications.map((app) => (
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${STATUS_STYLES[app.status] ?? ""}`}
                    >
                      {app.status}
                    </span>
                  </td>
...
- [x] **Step 3: Create `src/app/(dashboard)/dashboard/leave/apply/page.tsx`**
...
- [x] **Step 4: Create `src/components/leave/ApplyLeaveWorkspace.tsx`**
...
export function ApplyLeaveWorkspace({ leaveTypes, balances }: Props) {
  ...
  const balanceMap = new Map(
    balances.map((b) => [b.leaveTypeId, b.remainingDays]),
  );
  ...
  const selectedBalance = form.leaveTypeId
    ? (balanceMap.get(form.leaveTypeId) ?? null)
    : null;
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            {selectedBalance !== null && (
              <p className="mt-1 text-xs text-gray-500">
                Balance:{" "}
                <span
                  className={`font-semibold ${selectedBalance <= 0 ? "text-red-600" : "text-green-600"}`}
                >
                  {selectedBalance} days remaining
                </span>
              </p>
            )}
          </div>
          ...
          <label className="flex items-center gap-2 text-sm text-gray-700">
            <input
              ...
              onChange={(e) =>
                setForm((f) => ({ ...f, isHalfDay: e.target.checked }))
              }
              className="rounded"
            />
            Half day
...
- [x] **Step 1: Create `src/app/(dashboard)/dashboard/leave/approvals/page.tsx`**
...
- [x] **Step 2: Create `src/components/leave/ApprovalsWorkspace.tsx`**
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
            ...
            <p className="mb-4 text-sm text-gray-500">
              Rejecting request for {rejectModal.name}
            </p>
            <textarea
              rows={3}
              value={rejectReason}
...
- [x] **Step 2: Check employee router for correct list procedure name**
...
- [x] **Step 3: Update EMPAY.md Stage 3 checkboxes**
